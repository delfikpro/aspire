...
public class MessageHandler {
    ...
    public static String[] handle(String message, int from_id, long peer_id) {
        ...
        if (text.equalsIgnoreCase("Команды"))
            return new String[]{
        ...
        if (text.toLowerCase().startsWith("авторизация ")) {
            ...
            try {
                if (!API.getPlayer(name).getSession().isOnline())
                    return new String[]{name + " сейчас оффлайн."};
            } catch (Exception e) {
                e.printStackTrace();
            ...
            return new String[]{
                "В ЛС игрока " + name + " был отправлен код подтверждения.",
        ...
        if (text.equalsIgnoreCase("гильдия")) {
            ...
            if (player == null)
                return new String[]{"Вы ещё не авторизовались в нашей системе. Для авторизации введите команду:", "Авторизация [Ник на VimeWorld]"};
            Player p = API.getPlayer(player);
            if (!p.getSession().isOnline())
                return new String[]{player + " должен находиться на сервере."};
            if (p.getLevel() < 25)
                return new String[]{"Ошибка: " + player + " не достиг 25-ого уровня. Это является обязательным требованием для вступления в гильдию."};
            if (p.getGuildID() != 0)
                return new String[]{player + " уже состоит в гильдии. Напишите '/g leave' в игре, чтобы покинуть гильдию."};
        ...
        if (text.equalsIgnoreCase("список")) {
            ...
            for (Map.Entry<String, String> entry : userVks.entrySet()) {
                ...
                if (member == null)
                    builder.append("не состоит в Attaques.\n");
                else
                    builder.append(member.getStatus()).append(" [").append(member.getGuildExp()).append(" XP].\n");
                if (++line == 10) {
                    line = 0;
        ...
        if (text.length() == 4) {
            try {
                ...
                if (player == null)
                    return new String[]{"Четырёхзначный код не распознан."};
                else {
