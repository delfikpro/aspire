...
public class Sholimp {
	...
	private void handleLogLine(String line) {
		if (line.startsWith("Sends a private")) advertising = !advertising;
		if (line.startsWith("Request to teleport")) vimeBot.getVirtualPlayer().unlock();
		if (line.startsWith("/region <")) new Reconnector().reconnect(vimeBot.getVirtualPlayer().getRobot());
		if (line.startsWith("You have uninvited ")) handleUnInvite(line);
		boolean b = false;
		...
		for (String player : list) {
			...
			if (blacklist.contains(player)) continue;
	...
	private void handleUnInvite(String line) {
		...
		if (!blacklist.add(player)) return;
		try {
	...
	private void requestInvite(String player) {
		for (String warp : warps) vimeBot.getVirtualPlayer().queueCommand(WARP, this::successInvite, player, warp);
	}

	private void successInvite(Command command) {
		String player = command.getArgs()[0];
		if (!invited.add(player)) return;
		try {
