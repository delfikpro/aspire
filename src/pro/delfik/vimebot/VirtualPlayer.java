...
public abstract class VirtualPlayer extends Thread {
	...
	public VirtualPlayer(File logFile, Consumer<String> loggerCallback) {
		if (logFile == null) logInterceptor = null;
		else logInterceptor = new LogInterceptor(logFile, s -> handleLog(s, loggerCallback));
	}
	...
	private void handleLog(String s, Consumer<String> loggerCallback) {
		if (loggerCallback != null) loggerCallback.accept(s);
		if (currentCommand == null) return;
		if (!currentCommand.getPattern().responseMatches(s)) return;
	...
	public synchronized void start() {
		super.start();
		if (logInterceptor != null) logInterceptor.start();
	}
